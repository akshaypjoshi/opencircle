...
export class InviteCodesRouter extends BaseRouter {
  ...
  async update(inviteCodeId: string, data: InviteCodeUpdate): Promise<InviteCode> {
    return this.client.put<InviteCode>(`invite-codes/${inviteCodeId}`, data);
  }

  async deactivate(inviteCodeId: string): Promise<InviteCode> {
    return this.client.post<InviteCode>(`invite-codes/${inviteCodeId}/deactivate`);
  }

  async delete(inviteCodeId: string): Promise<{ message: string }> {
    return this.client.delete<{ message: string }>(`invite-codes/${inviteCodeId}`);
  }

  async validate(data: InviteCodeValidateRequest): Promise<InviteCodeValidateResponse> {
    return this.client.post<InviteCodeValidateResponse>('invite-codes/validate', data);
  }

  async getUsageStats(inviteCodeId: string): Promise<InviteCodeUsageStats> {
    return this.client.get<InviteCodeUsageStats>(`invite-codes/${inviteCodeId}/stats`);
  }
}
