...
export class CoursesRouter extends BaseRouter {
  // Course operations
  async getAllCourses(skip: number = 0, limit: number = 100, instructorId?: string, status?: string): Promise<Course[]> {
  ...
  // Section operations
  async getAllSections(courseId?: string, skip: number = 0, limit: number = 100): Promise<Section[]> {
    const params = new URLSearchParams({ skip: skip.toString(), limit: limit.toString() });
  ...
  async updateSection(sectionId: string, data: SectionUpdate): Promise<Section> {
  ...
  // Lesson operations
  async getAllLessons(sectionId?: string, skip: number = 0, limit: number = 100, type?: string): Promise<Lesson[]> {
    const params = new URLSearchParams({ skip: skip.toString(), limit: limit.toString() });
  ...
  async getAllEnrollments(userId?: string, courseId?: string, status?: string, skip: number = 0, limit: number = 100): Promise<EnrolledCourse[]> {
    const params = new URLSearchParams({ skip: skip.toString(), limit: limit.toString() });
    if (userId) params.append('user_id', userId);
    if (courseId) params.append('course_id', courseId);
    if (status) params.append('status', status);
    return this.client.get<EnrolledCourse[]>(`enrollments/?${params.toString()}`);
  }
  ...
  async updateEnrollment(enrollmentId: string, data: EnrolledCourseUpdate): Promise<EnrolledCourse> {
  ...
  async deleteEnrollment(enrollmentId: string): Promise<{ message: string }> {
    return this.client.delete<{ message: string }>(`enrollments/${enrollmentId}`);
  }
