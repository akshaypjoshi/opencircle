...
import type { User, UserCreate, UserUpdate, UserUpdateWithFile } from '../../types';

export class UsersRouter extends BaseRouter {
  async getAll(skip: number = 0, limit: number = 100, query?: string): Promise<User[]> {
  ...
  async updateWithFile(userId: string, data: UserUpdateWithFile, file?: File): Promise<User> {
    return this.client.putWithFiles<User>(`users/${userId}/with-file`, data, file);
  }
